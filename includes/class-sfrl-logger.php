...
class SFRL_Logger {
	...
	public function detect_404() {
		...
		$enable_logging = get_option( 'sfrl_enable_logging', true );
		if ( ! $enable_logging ) {
			return;
		}
	...
	private function log_404_request() {
		...
		// Skip admin or AJAX requests
		if ( is_admin() || defined( 'DOING_AJAX' ) ) return;

		if ( empty( $missing_url ) ) return;
		...
		if ( ! empty( $excluded_urls ) ) {
			...
			foreach ( $excluded_list as $excluded ) {
				if ( strpos( $missing_url, $excluded ) !== false ) {
					return; // Do not log
				}
			}
		}
		...
		if ( ! empty( $excluded_ips ) ) {
			...
			if ( in_array( $ip_address, $ip_list, true ) ) {
				return; // Do not log this request
			}
		}
	...
	public function delete_old_logs() {
		...
		$days = intval( get_option( 'sfrl_delete_after_days', 0 ) );
